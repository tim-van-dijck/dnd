export const Journal = {
    ...
    actions: {
        load({ state }) {
            axios.get('/api/campaign/journal')
                .then((response) => {
                    state.entries = response.data
                })
        },
        find({ commit }, entryId) {
            return axios.get(`/api/campaign/journal/${entryId}`)
                .then((response) => {
                    return response.data
                })
        },
        store({ state, dispatch }, entry) {
            return axios.post('/api/campaign/journal', entry)
                .then((response) => {
        ...
        update({ state, dispatch }, data) {
            ...
            payload._method = 'put'
            return axios.post(`/api/campaign/journal/${data.id}`, payload)
                .then((response) => {
        ...
        destroy({ dispatch }, entry) {
            return axios.delete(`/api/campaign/journal/${entry.id}`)
                .then(() => {
        ...
        sort({ state }, event) {
            ...
            axios.post('/api/campaign/journal/sort', { list: order })
                .then(() => {
                    for (const entry of state.entries) {
