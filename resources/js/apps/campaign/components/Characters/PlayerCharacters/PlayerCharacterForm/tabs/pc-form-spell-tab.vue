    ...
    <div id="spell-tab">
        ...
        <div uk-grid>
            <div class="uk-width-2-3@s spells-known"
                 v-if="selection.cantrips.length > 0 || selection.spells.length > 0">
                <h2>Spells known</h2>
                <div v-if="level.spells.length > 0 || (index == 0 && raceCantrip)" class="spell-level"
                     v-for="(level, index) in spellsKnown">
                    <h3>{{ level.title }}</h3>
                    <div class="uk-child-width-1-2@m uk-grid-small uk-grid-match" uk-grid>
                        <div v-for="spell in level.spells">
                            <div class="uk-card uk-card-body uk-card-primary">
                                ...
                                <button
                                    v-if="selection[spell.level > 0 ? 'spells' : 'cantrips'].find(item => spell.id == item.id)"
            ...
            <div class="uk-width-1-3@s">
                ...
                <div v-if="Object.keys(spells.items).length > 0 || (level == 0 && raceCantrips.length > 0)"
                    ...
                    <div :class="{'uk-form-horizontal': chosenClasses.length > 1 || raceCantrips.length > 0}">
                        ...
                        <div class="uk-margin" v-if="classSpells[level].items[chosenClass.id]"
                             v-for="chosenClass in chosenClasses">
                                    ...
                                    <option value="">
                                        - Make a choice ({{
                                            `${chosenClass.currentLevel[(
                                                level > 0
                                            ) ? 'spells_known' : 'cantrips_known'] - selection[level > 0
                                                ? 'spells'
...
export default {
    ...
    methods: {
        formatSpell(spell) {
            ...
            switch (formatted.origin_type) {
                case 'background':
                    ...
                    break
                case 'class':
                    formatted.origin_name = this.classes[formatted.origin_id] ? this.classes[formatted.origin_id].name
                        || 'Class' : 'Class'
                    break
                case 'subclass':
                    ...
                    break
                case 'race':
                    formatted.origin_name = (
                        this.race || {}
                    ).id === spell.origin_id ? this.race.name || 'Race' : 'Race'
                    break
                case 'subrace':
                    formatted.origin_name = (
                        this.subrace || {}
                    ).id === spell.origin_id ? this.subrace.name || 'Race' : 'Race'
                    break
            }
        ...
        removeRaceCantrip() {
            if (this.raceCantrip) {
                let existingIndex = this.selection.cantrips.findIndex(item => item.level
                    == 0
                    && item.origin_type
                    == 'Race')
        ...
        classSpells() {
            ...
            for (let level = 0; level < 10; level++) {
                ...
                for (let charClass of this.chosenClasses) {
                    ...
                    if (type == 'cantrips') {
                        canSelect = (
                            level == 0 && charClass.currentLevel['cantrips_known'] > 0
                        )
                    } else {
                        canSelect = this.spellsAvailable.maxSpellLevel
                            >= level
                            && charClass.currentLevel[`${type}_known`]
                            > 0
                    }
                    if (canSelect) {
                        levelSpells.items[charClass.id] = this.spells
                            .filter(item => {
                                let selection = this.selection[level > 0
                                    ? 'spells'
                                    : 'cantrips'].find(selected => selected.id == item.id)
                                return selection
                                    == null
                                    && item.level
                                    === level
                                    && this.classes[charClass.id].spells.includes(item.id)
                            })
                            ...
                            if (charClass.subclass.spells) {
                                let subclassSpells = this.spells.filter(item => {
                                    ...
                                    return selection
                                        ...
                                        && item.level
                                        === level
                                        && charClass.subclass.spells.includes(item.id)
                                })
        ...
        classLevels() {
            ...
            for (let charClass of this.characterClasses) {
                if (charClass.class_id) {
                    ...
                    let subclass = this.classes[charClass.class_id].subclasses.find(item => item.id
