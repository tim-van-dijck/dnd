        ...
        <div id="add-armor-modal" uk-modal @beforehide="armorId = 0; quantity = 1">
            <div class="uk-modal-dialog uk-modal-body">
                ...
                <template v-if="selectedArmor !== null">
                    <hr>
                    <div class="uk-margin">
                        <h3>{{ selectedArmor.name }}<template v-if="selectedArmor.type !== 'Shield'"> ({{ selectedArmor.type }})</template></h3>
...
export default {
    ...
    created() {
        this.$store.dispatch('Inventory/loadItems', 'armor')
    },
    data() {
        return {
            quantity: 1,
    ...
    methods: {
        addArmor() {
            this.$store.dispatch('Inventory/add', {inventoryId: this.inventoryId, item: this.selectedArmor, quantity: this.quantity})
                .then(() => {
