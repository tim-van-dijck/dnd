<template>
    <div id="race-list" v-if="info.races">
                ...
                <div uk-grid>
                    <div class="uk-width-1-5">
                        <ul class="uk-nav uk-nav-default">
                            <li :class="{'uk-active': state.active.race?.id === race?.id}"
                                ...
                                <div class="uk-width-1-3">
                                    <ul class="uk-nav uk-nav-default">
                                        <li :class="{'uk-active': state.active.trait === trait.id}"
                                            v-for="trait in info.activeRace.value.traits">
                            ...
                            <div v-show="state.active.tab === 'subraces'">
                                <ul uk-tab>
                                    <li :class="{'uk-active': state.active.subrace === subrace.id}"
                                        v-for="subrace in info.activeRace.value.subraces">
                                        <a href="#" @click.prevent="state.setActiveSubrace(subrace.id)">{{
                                                subrace.name
                                            }}</a>
                                    </li>
                                </ul>
                                <div v-if="state.active.subrace === subrace.id" class="subrace-info"
                                     v-for="subrace in info.activeRace.value.value.subraces">
                                    ...
                                    <ul class="uk-list">
                                        ...
                                        <li v-if="info.activeSubrace.value.proficiencies.tools.length > 0"><b>Tools:</b>
                                        ...
                                        <li v-if="info.activeSubrace.value.proficiencies.armor.length > 0"><b>Armor:</b>
...
export default {
    ...
    setup() {
        ...
        watch(() => races, () => state.setActive(races?.[0]?.id || null))
